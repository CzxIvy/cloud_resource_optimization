...
const Dashboard = () => {
  ...
  const handleWebSocketData = (data) => {
    ...
    if (data.resource_status) {
      ...
      const utilizationHistory = data.resource_status.map(resource => {
        return {
          ...resource,
          history: resource.history || Array.from({ length: 30 }, (_, i) => ({
    ...
    if (data.task_status) {
      ...
      const queueStatus = {
        ...
        completed: data.task_status.filter(t => t.status === 'completed').length,
      ...
      // 计算平均等待时间
      const pendingTasks = data.task_status.filter(t => t.status === 'pending' && t.waiting_time);
      if (pendingTasks.length > 0) {
        queueStatus.average_wait_time = pendingTasks.reduce((sum, task) => sum + task.waiting_time, 0) / pendingTasks.length;
      }

      // 计算平均执行时间
      const completedTasks = data.task_status.filter(t => t.status === 'completed' && t.execution_time);
      if (completedTasks.length > 0) {
        queueStatus.average_execution_time = completedTasks.reduce((sum, task) => sum + task.execution_time, 0) / completedTasks.length;
      }
  ...
  const fetchData = async () => {
    try {
      ...
      // 获取资源利用率（默认方式）
      const utilizationResponse = await fetch('http://localhost:8000/metrics/resource-utilization');
      ...
      // 获取任务队列状态
      const queueResponse = await fetch('http://localhost:8000/metrics/task-queue');
      if (queueResponse.ok) {
  ...
  useEffect(() => {
    ...
    // 原有的模拟数据逻辑
    const mockResources = [
      { id: 1, name: 'Server-A', type: 'cpu', capacity: 16, used: 12, utilization: 75, status: 'available' },
      { id: 2, name: 'Server-B', type: 'cpu', capacity: 32, used: 8, utilization: 25, status: 'available' },
      { id: 3, name: 'GPU-Cluster-1', type: 'gpu', capacity: 8, used: 7, utilization: 87.5, status: 'available' },
      { id: 4, name: 'Memory-Node-1', type: 'memory', capacity: 128, used: 64, utilization: 50, status: 'available' },
    ];

    const mockTasks = [
      { id: 1, name: 'ML训练任务-1', type: 'training', status: 'running', priority: 3, resource_id: 3, waiting_time: null, execution_time: 120 },
      { id: 2, name: '数据处理-1', type: 'processing', status: 'pending', priority: 2, resource_id: null, waiting_time: 45, execution_time: null },
      { id: 3, name: '日志分析-1', type: 'analysis', status: 'completed', priority: 1, resource_id: 1, waiting_time: null, execution_time: 78 },
      { id: 4, name: 'API服务-1', type: 'service', status: 'running', priority: 3, resource_id: 2, waiting_time: null, execution_time: 360 },
      { id: 5, name: '数据库备份', type: 'backup', status: 'pending', priority: 1, resource_id: null, waiting_time: 12, execution_time: null },
    ];
    ...
    const timer = setInterval(() => {
      ...
      setResources(prev => prev.map(resource => ({
        ...resource,
        used: Math.min(resource.capacity, resource.used + (Math.random() > 0.5 ? 1 : -1)),
        utilization: Math.min(100, Math.max(0, resource.utilization + (Math.random() > 0.5 ? 5 : -5)))
      })));
      ...
      setTasks(prev => {
        ...
        if (updated.length > 0) {
          ...
          if (updated[index].status === 'pending') {
            ...
            updated[index].execution_time = Math.floor(Math.random() * 100) + 50;
          }
        }
        return updated;
      });
      ...
      // 更新利用率历史
      setResourceUtilization(prev => prev.map(resource => {
  ...
  const handleDeleteResource = async (resourceId) => {
    if (!window.confirm('确定要删除此资源吗？如果有任务正在使用此资源，将无法删除。')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setResourceUtilization(prev => prev.filter(resource => resource.id !== resourceId));
        alert('资源删除成功');
      } else {
        // 尝试解析错误信息
    ...
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="text-3xl font-bold">{taskQueueStatus.average_wait_time.toFixed(1)}秒</div>
        </div>
        <div className="bg-white p-4 rounded shadow">
          <div className="text-gray-500 mb-2">平均执行时间</div>
          <div className="text-3xl font-bold">{taskQueueStatus.average_execution_time.toFixed(1)}秒</div>
        </div>
      </div>
            ...
            <div key={resource.id} className="border p-3 rounded">
              ...
              <ResponsiveContainer width="100%" height={100}>
                <LineChart data={resource.history}>
                  ...
                  <Tooltip formatter={(value) => [`${value.toFixed(1)}%`, '利用率']} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          ))}
              ...
              {tasks.map(task => (
                <tr key={task.id}>
                  ...
                  <td className="py-2 px-4 border-b">
                    <span className="px-2 py-1 rounded text-sm" style={{ backgroundColor: `${statusColors[task.status]}20`, color: statusColors[task.status] }}>
                      {task.status === 'pending' ? '待处理' : task.status === 'running' ? '运行中' : task.status === 'completed' ? '已完成' : '失败'}
                    </span>
                  </td>
                  <td className="py-2 px-4 border-b">{task.priority}</td>
                  <td className="py-2 px-4 border-b">{task.resource_id ? resources.find(r => r.id === task.resource_id)?.name || task.resource_id : '-'}</td>
                  <td className="py-2 px-4 border-b">
                    {task.waiting_time ? `等待: ${task.waiting_time}秒` : task.execution_time ? `执行: ${task.execution_time}秒` : '-'}
                  </td>
                </tr>
              ))}
            </tbody>
