...
def get_traj(test_type, pa, env, episode_max_length, pg_resume=None, render=False):
    ...
    for _ in range(episode_max_length):
        ...
        if test_type == 'Tetris':
            a = other_agents.get_packer_action(env.machine_cluster, env.task_slot)

        elif test_type == 'SJF':
            a = other_agents.get_sjf_action(env.machine_cluster, env.task_slot)

        elif test_type == 'Random':
            a = other_agents.get_random_action(env.task_slot)

        ob, rew, done, info = env.schedule_step(a)
...
def launch(pa, pg_resume=None, render=False, plot=False, repre='compact', end='no_new_job'):
    ...
    for seq_idx in range(pa.num_ex):
        ...
        for test_type in test_types:
            ...
            rews, info = get_traj(test_type, pa, envs[seq_idx], pa.episode_max_length, pg_resume)
