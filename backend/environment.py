...
job_names = ["数据处理-1", "日志分析-1", "数据备份-1", "日志分析-2", "数据处理-2", "数据备份-2", "数据备份-3", "数据处理-3"]
job_types = ["processing", "analysis", "backup", "analysis", "processing", "backup", "backup", "processing"]
...
class Env:
    def __init__(self, pa, job_seq = None,
        ...
        if job_seq is None:
            self.pa.num_ex = 1
            self.job_seq = job_distribution.generate_sequence_work(self.pa)[0]
            print(self.job_seq)
            idx = 0
            for i in range(len(self.job_seq)):
                ...
                if j is None:
                    ...
                    continue
                job = Job(id=i, name=job_names[name_id], type=job_types[name_id])
                name_id += 1
                for k in range(len(j[0])):
                    task = Task(seq_idx=idx, job_id=i, task_len=j[0][k], res_vec=j[1][k])
                    self.Tasks.append(task)
                    job.task_idx.append(idx)
                    job.resource_requirements["cpu"] += j[1][k][0]
                    job.resource_requirements["memory"] += j[1][k][1]
                    idx += 1
    ...
    def schedule_step(self, a):
        ...
        if self.seq_idx >= len(self.nw_len_seqs):
            l = [i for i in range(self.pa.num_nw) if self.task_slot.slot[i] is not None]
            if len(l) > 0:
                if self.task_slot.slot[l[0]] is not None:
                    allocated = self.machine_cluster.allocate_task(self.task_slot.slot[l[0]], self.curr_time)
                    if not allocated:
                        status = "MoveOn"
                    else:
                        status = "Allocate"
        ...
        if status == 'MoveOn':
            time.sleep(1)
            self.curr_time += 1
            self.machine_cluster.time_proceed(self.curr_time, self.Jobs)
            ...
            for i in range(2):
                ...
                self.resource_history[i].append({
                    "time": (basetime + timedelta(seconds=self.curr_time)).isoformat(),
                    "value": utilization,
                })

                # 只保留最近30秒的历史
                cutoff_time = (basetime + timedelta(seconds=self.curr_time) - timedelta(seconds=30)).isoformat()
            ...
            if self.seq_idx >= len(self.nw_len_seqs):
                if len(self.machine_cluster.running_task) == 0 and \
                    ...
                    print(len(self.machine_cluster.running_task))
                    l = [i for i in range(self.pa.num_nw) if self.task_slot.slot[i] is not None]
                    aa = [s.len for s in self.task_slot.slot if s is not None]
                    c = [s.res_vec for s in self.task_slot.slot if s is not None]
                    print(a, l, aa, c)
            ...
            if not done:
                if self.seq_idx < len(self.nw_len_seqs):  # otherwise, end of new Task sequence, i.e. no new Tasks
                    ...
                    if new_task.len > 0:  # a new task comes
                        if self.Jobs[new_task.job_id].enter_time == -1:
    ...
    def allocate_step(self, a):
        ...
        cutoff_time = (basetime + timedelta(seconds=self.curr_time) - timedelta(seconds=30)).isoformat()
        self.cost_history[0] = [
            h for h in self.cost_history[0]
            if h["time"] > cutoff_time
        ]
    ...
    def get_job_status(self):
        ...
        result = [
            ...
            for job in self.Jobs if job is not None and job.enter_time != -1
        ]
    ...
    def get_cost(self):
        return [
            {
                "cost": self.cost,
                "cost_history": self.cost_history.get(0, [])
            }
        ]
    ...
    def get_job_waiting_time(self, job):
        ...
        if job.start_time == -1:
            ...
            return job.start_time - job.enter_time
    ...
    def get_resource_utilization(self):
        ...
        for i in range(2):
            # 这里的task指Job
            task_count = len([t for t in self.Jobs if t is not None and t.get_status() == "running"])
            result.append({
                "id": i,
                "name": names[i],
                "type": types[i],
    ...
    def get_task_queue_status(self):
        ...
        return {
            "pending": len([t for t in self.Jobs if t is not None and t.get_status() == "pending"]),
            "running": len([t for t in self.Jobs if t is not None and t.get_status() == "running"]),
            "completed": len([t for t in self.Jobs if t is not None and t.get_status() == "completed"]),
            "failed": 0,
    ...
    def get_canvas(self):
        result = {
            "canvas": self.machine_cluster.canvas
        }
        return convert_numpy_to_python(result)
...
class Job:
    def __init__(self, id, name, type):
        ...
        self.start_time = -1
        self.finish_time = -1
        self.resource_requirements = {"cpu": 0, "memory": 0}
        self.name = name
        self.type = type
...
class MachineCluster:
    ...
    def time_proceed(self, curr_time, jobs):
        ...
        for task in self.running_task:
            ...
            if task.finish_time <= curr_time:
                ...
                if task.seq_idx == max(jobs[task.job_id].task_idx):
                    jobs[task.job_id].finish_time = task.finish_time
