...
def generate_task_seq(job_seq):
    nw_len_seqs = np.empty((0,), dtype=int)
    nw_size_seqs = np.empty((0, 2), dtype=int)
    for job in job_seq:
        if job is None:
            nw_len_seqs = np.append(nw_len_seqs, 0)
            nw_size_seqs = np.append(nw_size_seqs, [[0, 0]], axis=0)
            continue
        print(job)
        nw_len_seqs = np.append(nw_len_seqs, job[0])
        nw_size_seqs = np.append(nw_size_seqs, job[1], axis=0)
    # print(nw_len_seqs)
    # print(nw_size_seqs)
